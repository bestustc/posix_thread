...
// Both threads access the same variable and use different mutexes to lock in
// order to solve data-race condition. Each thread locks mutex and then
// performs the work. It unlocks mutex at the end.
//
// This executable demonstrates, that with current approach reader is going to
// see changes to the variable, b/c writer uses it's own lock.
//
...
int main(int argc, char *argv[])
{
    ...
    cout << "start threads using different lock" << endl;
    ...
    //
    if (0 != pthread_create(&tid_reader, NULL, reader, &lock_a))
    {
    ...
    if (0 != pthread_create(&tid_writer, NULL, writer, &lock_b))
    {
